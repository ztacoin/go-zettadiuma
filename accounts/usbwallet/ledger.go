...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Zettadium app in browser mode", w.failure
	}
	if w.offline() {
		return "Zettadium app offline", w.failure
	}
	return fmt.Sprintf("Zettadium app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
