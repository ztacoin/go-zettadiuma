...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gzta --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gzta --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:9019"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.zettadium
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["9019/tcp"]
