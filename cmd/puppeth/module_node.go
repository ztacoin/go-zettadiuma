...
{{end}}
RUN \
  echo 'gzta --cache 512 init /genesis.json' > gzta.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.zettadium/keystore/ && cp /signer.json /root/.zettadium/keystore/' >> gzta.sh && \{{end}}
	echo $'exec gzta --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gzta.sh

ENTRYPOINT ["/bin/sh", "gzta.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.zettadium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.ztahash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.zettadium"],
		ethashdir:  infos.volumes["/root/.ztahash"],
		port:       port,
